...
%         plot(set_numpoints, CI_w(kk,:,j+3), 'o--', set_numpoints, CI_w(kk,:,j), 'd--',  'MarkerFaceColor', 'c', 'MarkerSize',8,'LineWidth', 1.5);
    plot(set_numpoints, CI_w(:,j+0), '*-',set_numpoints,CI_w(:,j+num_param*(num_types-2)), 's:',...
        set_numpoints, CI_w(:,j+num_param*(num_types-1)), 'o--', ...
        'MarkerFaceColor', 'c', 'MarkerSize',8,'LineWidth', 1.5);

%         plot(set_numpoints, CI_w(:,j+0), '*-',set_numpoints,CI_w(:,j+num_param*(num_types-1)), 'o--',...
%         'MarkerFaceColor', 'c', 'MarkerSize',8,'LineWidth', 1.5);        
    ...
    %         legend('Evenly','Optimized FIM','Optimized Sob','Box', 'off'); 
        legend('Evenly','IID FIM','OU FIM', 'Location', 'best','Box', 'off'); 
        end
...
type_name={'Evenly','IID FIM','OU FIM'};
...
colors = lines(num_types); % Using lines colormap to generate distinct colors
 ...
 fig = figure('Position', [20 20 1000 320], 'color', 'w');    
for j = 1:num_param
          subplot('Position',[0.05+(j-1)*0.315,0.13,0.26,0.75]);
...
%                ylabel(sprintf('Parameter %s', prm_name{j}));
               ylabel('Parameter value');
                legend('Evenly','IID FIM','OU FIM', 'Location', 'best','Box', 'off'); % Add legend
        
%                legend('Evenly','Optimized FIM','Optimized Sob', 'Location', 'best','Box', 'off'); % Add legend
           end
            h1=plot([set_numpoints,set_numpoints(end)+0.2], linspace(prm(j), prm(j), length(set_numpoints)+1),...
