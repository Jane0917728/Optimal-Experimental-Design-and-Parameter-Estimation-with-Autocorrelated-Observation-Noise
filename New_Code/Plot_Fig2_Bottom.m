%Main program to compute the average CI for evently distributed points and
%let sigmaIID=sigmaOU Figure3
% clear;
clc;
tic
...
prm_all=[r K N0]'; prm_name_all = {'r', 'K', 'C_0'};   
 n_s=10;%number of measurement points
...
for k=1:num_param
  ...
  plot(Sigma, CI_w(:,k),'d:',Sigma, CI_w(:,k+num_param*1),  '--+', Sigma, CI_w(:,k+num_param*2), '-*', 'MarkerFaceColor', 'c', 'MarkerSize',8,'LineWidth', 1.5);
