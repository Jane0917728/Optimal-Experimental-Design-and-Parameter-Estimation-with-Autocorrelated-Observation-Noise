...
% phi=[exp(linspace(log(0.02),log(2),num_phi))]';
% phi=linspace(0.02,2,num_phi)';
SigmaC=2;
% num_phi=12;
...
t_vec={t_best_OUFIM,t_best_OUSob};
%  t_vec={t_best_OUFIM,t_best_OUSob};
...
for k=1:num_types
    ...
    for j=1:l_set 
        ...
        %SIG_pinv=pinv(SIG_OU);
        phi1=phi(j);
        Sigma1=Sigma(j);
        %mu_OU=x_0*exp(-phi(j)*t_opt)+mu*(1-exp(-phi(j)*t_opt));
        parfor i=1:ave_num   
    %                 measurements=N_real+G_exact_OUmulti(phi(i),mu,Sigma(j),x_0,t_opt,Z{k}(i,:));     
             measurements=N_real+Generate_exact_OU(phi1,Sigma1,t_opt,t_initial);
...
%       plot(phi, CI_w(:,k),  'o--',  'MarkerFaceColor', 'c', 'MarkerSize',8,'LineWidth', 1.5);
% xlim([set_numpoints(1),set_numpoints(end)]);
       plot(phi, CI_w(:,k),  'o--', phi, CI_w(:,k+num_param*1), 'd:', 'MarkerFaceColor', 'c', 'MarkerSize',8,'LineWidth', 1.5);
...
%   legend('IID FIM','OU FIM','OU Sobol');
    legend('OU FIM','OU Sobol');  % If there are 5 points, set OU FIM5, otherwise set OU FIM 10
    title(sprintf('Parameter %s', prm_name{k}));   
end
% t_vec={t_best_IIDFIM,t_best_OUFIM,t_best_OUSob};
% fig=figure('Position',[20 20 1000 320],'color','w');
%   %   [left bottom width height]
% for j=1:num_param
...
% %     plot(set_numpoints, MSE_param(:,j), '*-',set_numpoints, MSE_param(:,j+num_param*max(0,(num_types-2))), 'o--',set_numpoints, MSE_param(:,j+num_param*(num_types-1)), 'd-',  'MarkerFaceColor', 'c', 'MarkerSize',8,'LineWidth', 1.5);
% %       plot(set_numpoints, MSE_param(:,j), 'x-',set_numpoints, MSE_param(:,j+num_param*1),'*-',...
% %         set_numpoints, MSE_param(:,j+num_param*2),  'o--',set_numpoints, MSE_param(:,j+num_param*3), 'd-',  'MarkerFaceColor', 'c', 'MarkerSize',8,'LineWidth', 1.5);
%        plot(set_numpoints, MSE_param(:,j), '*-',set_numpoints, MSE_param(:,j+num_param*max(0,(num_types-2))), 'o--',...
%            set_numpoints, MSE_param(:,j+num_param*(num_types-1)), 'd:',  'MarkerFaceColor', 'c', 'MarkerSize',8,'LineWidth', 1.5);
%   
%     
%     xlim([set_numpoints(1),set_numpoints(end)]);
